...
def test_model(args, test_pairs, generate_num_ids, encoder, predict, generate, merge, output_lang, beam_size):
    ...
    for epoch in epochs:
        ...
        for test_pair in test_pairs:
            ...
            for idx, test_batch in enumerate(test_pair):
                test_res = evaluate_tree(test_batch[0], test_batch[1], generate_num_ids, encoder, predict, generate,
                                        merge, output_lang, test_batch[5], beam_size=beam_size)
                val_ac, equ_ac, test_, tar_ = compute_prefix_tree_result(test_res, test_batch[2], output_lang, test_batch[4], test_batch[6])
                
                if val_ac:
                    value_ac += 1
                else:
                    indices.append(idx)
                    mi_type.append('value')
                    results.append(test_)
                    targets.append(tar_)
                if equ_ac:
                    equation_ac += 1
                else:
                    indices.append(idx)
                    mi_type.append('equation')
                    results.append(test_)
                    targets.append(tar_)
                del test_res
                eval_total += 1
