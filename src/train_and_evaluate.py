...
def mask_num(encoder_outputs, decoder_input, embedding_size, nums_start, copy_nums, num_pos):
    ...
    if USE_CUDA:
        indices = indices.cuda(device = torch.device(os.environ["DEVICE"]))
    batch_size = decoder_input.size(0)
    sen_len = encoder_outputs.size(0)
    batch_num = torch.LongTensor(range(batch_size))
    batch_num = batch_num * sen_len
    if USE_CUDA:
        batch_num = batch_num.cuda(device = torch.device(os.environ["DEVICE"]))
...
def get_all_number_encoder_outputs(encoder_outputs, num_pos, batch_size, num_size, hidden_size):
    ...
    if USE_CUDA:
        indices = indices.cuda(device = torch.device(os.environ["DEVICE"]))
        masked_index = masked_index.cuda(device = torch.device(os.environ["DEVICE"]))
...
def train_attn(input_batch, input_length, target_batch, target_length, num_batch, nums_stack_batch, copy_nums,
    ...
    if USE_CUDA:
        input_var = input_var.cuda(device = torch.device(os.environ["DEVICE"]))
        seq_mask = seq_mask.cuda(device = torch.device(os.environ["DEVICE"]))
    ...
    if USE_CUDA:
        all_decoder_outputs = all_decoder_outputs.cuda(device = torch.device(os.environ["DEVICE"]))

    if random.random() < use_teacher_forcing:
        # Run through decoder one time step at a time
        for t in range(max_target_length):
            if USE_CUDA:
                decoder_input = decoder_input.cuda(device = torch.device(os.environ["DEVICE"]))
        ...
        if USE_CUDA:
            score = score.cuda(device = torch.device(os.environ["DEVICE"]))
        ...
        for t in range(max_target_length):
            ...
            if USE_CUDA:
                beam_scores = beam_scores.cuda(device = torch.device(os.environ["DEVICE"]))
                all_hidden = all_hidden.cuda(device = torch.device(os.environ["DEVICE"]))
                all_outputs = all_outputs.cuda(device = torch.device(os.environ["DEVICE"]))
            ...
            for b_idx in range(len(beam_list)):
                ...
                if USE_CUDA:
                    rule_mask = rule_mask.cuda(device = torch.device(os.environ["DEVICE"]))
                    decoder_input = decoder_input.cuda(device = torch.device(os.environ["DEVICE"]))
    ...
    if USE_CUDA:
        target = target.cuda(device = torch.device(os.environ["DEVICE"]))
...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
    ...
    if USE_CUDA:
        input_var = input_var.cuda(device = torch.device(os.environ["DEVICE"]))
        seq_mask = seq_mask.cuda(device = torch.device(os.environ["DEVICE"]))
        padding_hidden = padding_hidden.cuda(device = torch.device(os.environ["DEVICE"]))
        num_mask = num_mask.cuda(device = torch.device(os.environ["DEVICE"]))
    ...
    for t in range(max_target_length):
        ...
        target[t] = target_t
        if USE_CUDA:
            generate_input = generate_input.cuda(device = torch.device(os.environ["DEVICE"]))
    ...
    if USE_CUDA:
        all_node_outputs = all_node_outputs.cuda(device = torch.device(os.environ["DEVICE"]))
        target = target.cuda(device = torch.device(os.environ["DEVICE"]))
...
def contra_train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
                      encoder, neural_sim, predict, generate, merge,
    ...
    if contra_loss_func == "":
        if len(input_batch) > 2:
            ...
            if USE_CUDA:
                y_target = y_target.cuda(device = torch.device(os.environ["DEVICE"]))
        ...
        out = neural_sim(torch.cat([problem_output_ls[0], problem_output_ls[1], problem_output_ls[2]], dim=1))
        pos_sim = out[:, 0]
        neg_sim = out[:, 1]
        # out = neural_sim(similarity_batch)
        # print("OUT", out.size())
        # sim_target = torch.ones(similarity_batch.size(0), device = torch.device(os.environ["DEVICE"]))
        # sim_target[target.size(0)//2:] = -1
        # print("Sim Target", sim_target.size())
        # mse_loss = torch.nn.MSELoss()
        # sim_loss = mse_loss(out, sim_target)
