...
from src.neuralsim import NeuralSimilarity
...
def initial_model(output_lang, embedding_size, hidden_size, args, copy_nums, generate_nums):
    encoder = EncoderBert(hidden_size=hidden_size, auto_transformer=True, 
                        bert_pretrain_path=args.bert_pretrain_path, dropout=args.dropout)
    neural_sim = NeuralSimilarity()
    ...
    if args.model_reload_path != '' and os.path.exists(args.model_reload_path):
        ...
        neural_sim.load_state_dict(torch.load(os.path.join(args.model_reload_path, "neural_sim.ckpt")))
        ...
        if args.finetune_from_trainset != "": # alignment finetune output vocab
            ...
            use_bert = True
            _, from_output_lang, _, _, _ = prepare_data(from_pairs_trained, (), 5, from_generate_nums, from_copy_nums, tree=True, use_bert=use_bert, auto_transformer=True, bert_pretrain_path=args.bert_pretrain_path)
            op_weight = None
            op_bias = None
            ...
            for i in generate_num_ids: # constant
                ...
                if constant not in from_output_lang.word2index:
                    ...
                    if USE_CUDA:
                        const_emb = const_emb.cuda(device = torch.device(os.environ["DEVICE"]))
                else:
    ...
    return encoder, neural_sim, predict, generate, merge

def train_model(args, train_pairs, test_pairs, generate_num_ids, 
                    encoder, neural_sim, predict, generate, merge, output_lang):
    batch_size = args.batch_size
    need_optimized_parameters = []
    for module in [encoder, neural_sim, predict, generate, merge]:
    ...
    for epoch in range(args.n_epochs):        
        ...
        for idx in range(len(input_lengths)):
            ...
            if args.contra_common_tree_pair:
                loss, contra_loss, expr_loss = contra_train_tree(
                    ...
                    num_stack_batches[idx], num_size_batches[idx], generate_num_ids, encoder, neural_sim, predict, generate, merge,
        ...
        if epoch % args.n_val == 0 or epoch > args.n_epochs - 5:
            ...
            if float(value_ac) / eval_total > min(best_value_acc_ls):
                ...
                torch.save(neural_sim.state_dict(), os.path.join(args.output_dir, "epoch_{}".format(epoch), "neural_sim.ckpt"))
...
if __name__=='__main__':
    ...
    parser.add_argument('--device', default='cpu', type=str, help="GPU Device number")
    ...
    input_lang, output_lang, train_pairs, (test_pairs1, val_pairs1, test_pairs2, val_pairs2), len_bert_token = prepare_data(pairs_trained, (pairs_tested1, pairs_valed1, pairs_tested2, pairs_valed2), 5, generate_nums,
                                                                                copy_nums, tree=True, use_bert=use_bert, auto_transformer=True, bert_pretrain_path=args.bert_pretrain_path)

    generate_num_ids = []
    for num in generate_nums:
        generate_num_ids.append(output_lang.word2index[num])
    encoder, neural_sim, predict, generate, merge = initial_model(output_lang, embedding_size, hidden_size, args, copy_nums, generate_nums)
    if torch.cuda.is_available():
        encoder.cuda(device = torch.device(os.environ["DEVICE"]))
        neural_sim.cuda(device = torch.device(os.environ["DEVICE"]))
        predict.cuda(device = torch.device(os.environ["DEVICE"]))
        generate.cuda(device = torch.device(os.environ["DEVICE"]))
        merge.cuda(device = torch.device(os.environ["DEVICE"]))
